"""Python wrapper for Obscura headless browser.

Provides a clean API for AI agents to fetch, extract, and interact with web pages.
Requires the obscura binary to be built and on PATH or at a known location.
"""
...
BINARY_LOCATIONS = [
    Path.home() / ".cargo" / "bin" / "obscura",
    Path(__file__).parent / "target" / "release" / "obscura",
]
...
def _find_binary() -> str:
    ...
    raise FileNotFoundError(
        "obscura binary not found. Build with: cd obscura && cargo build --release --features stealth"
    )
